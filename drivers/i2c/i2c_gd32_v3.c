...
int i2c_gd32_configure_gd(const struct device *dev, uint32_t dev_config)
{
	...
	if (GD32_I2C_IS_ADD(cfg->reg)) {
		...
		if (fast_like) {
			...
			/* SCL delay calculation: tSCLDELY ≥
			 * {tSU,DAT(min)}/[(PSC+1)*tI2CCLK]} - 1
			 */
			scl_dely = (tsu_dat_min_ns + (t_psc * t_i2c_clk_ns / 2)) /
				   (t_psc * t_i2c_clk_ns);
			if (scl_dely > 0U) {
